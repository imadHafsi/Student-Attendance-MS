...
8)
  , editlist = !1
  , options = {
    valueNames: ["id", "name", "company_name", "designation", "date", "email_id", "phone", "lead_score", "tags"],
    page: perPage,
    pagination: !0,
...
xhttp.onload = function() {
    ...
    Array.from(e).forEach(function(e) {
        ...
        Array.from(t).forEach( (e, t) => {
            a += '<span class="badge bg-primary-subtle text-primary me-1">' + e + "</span>"
        }
        ),
        contactList.add({
            id: `<a href="javascript:void(0);" class="fw-medium link-primary">#VZ${e.id}</a>`,
            name: '<div class="d-flex align-items-center">            <div class="flex-shrink-0"><img src="' + e.name[0] + '" alt="" class="avatar-xs rounded-circle"></div>            <div class="flex-grow-1 ms-2 name">' + e.name[1] + "</div>            </div>",
            company_name: e.company_name,
            desc: e.desc,
            designation: e.designation,
            date: formatDate(e.date) + ' <small class="text-muted">' + timeConvert(e.date) + "</small>",
            email_id: e.email_id,
            phone: e.phone,
...
var idField = document.getElementById("id-field")
  ...
  , table = (viewBtns = document.getElementsByClassName("view-item-btn"),
refreshCallbacks(),
document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
    e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Contact",
    document.getElementById("showModal").querySelector(".modal-footer").style.display = "block",
    document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Contact",
    document.getElementById("showModal").querySelector(".modal-footer").style.display = "block",
    document.getElementById("add-btn").innerHTML = "Add Contact") : (document.getElementById("exampleModalLabel").innerHTML = "List Contact",
...
var count = 11
  , tagInputField = new Choices("#taginput-choices",{
    removeItemButton: !0
})
  , tagInputFieldValue = tagInputField.getValue(!0)
  , tagHtmlValue = ""
  , forms = (Array.from(tagInputFieldValue).forEach( (e, t) => {
    tagHtmlValue += '<span class="badge bg-primary-subtle text-primary me-1">' + e + "</span>"
}
),
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    contactList.remove("id", t.outerHTML),
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                var t = (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML
                  , a = (new DOMParser).parseFromString(e._values.tags, "text/html").body.querySelectorAll("span.badge");
                t == itemId && (editlist = !0,
                idField.value = t,
                customerImg.src = (new DOMParser).parseFromString(e._values.name, "text/html").body.querySelector("img").src,
                customerNameField.value = (new DOMParser).parseFromString(e._values.name, "text/html").body.querySelector(".name").innerHTML,
                company_nameField.value = e._values.company_name,
                designationField.value = e._values.designation,
                email_idField.value = e._values.email_id,
                phoneField.value = e._values.phone,
                lead_scoreField.value = e._values.lead_score,
    ...
    Array.from(viewBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                document.getElementById("contact-view-detail").innerHTML = e)
            })
        })
    })
}
...
Array.prototype.slice.call(forms).forEach(function(n) {
    n.addEventListener("submit", function(e) {
        var t, a;
        n.checkValidity() ? (e.preventDefault(),
        "" === customerNameField.value || "" === company_nameField.value || "" === email_idField.value || "" === phoneField.value || "" === lead_scoreField.value || "" === designationField.value || editlist ? "" !== customerNameField.value && "" !== company_nameField.value && "" !== email_idField.value && "" !== phoneField.value && "" !== lead_scoreField.value && "" !== designationField.value && editlist && (t = contactList.get({
            id: idField.value
        }),
        Array.from(t).forEach(function(e) {
            ...
            Array.from(a).forEach( (e, t) => {
                n += '<span class="badge bg-primary-subtle text-primary me-1">' + e + "</span>"
            }
            ),
            t == itemId && e.values({
                id: `<a href="javascript:void(0);" class="fw-medium link-primary">#VZ${idField.value}</a>`,
                name: '<div class="d-flex align-items-center">                            <div class="flex-shrink-0"><img src="' + customerImg.src + '" alt="" class="avatar-xs rounded-circle object-fit-cover"></div>                            <div class="flex-grow-1 ms-2 name">' + customerNameField.value + "</div>                            </div>",
        ...
        contactList.add({
            ...
            name: '<div class="d-flex align-items-center">                    <div class="flex-shrink-0"><img src="' + customerImg.src + '" alt="" class="avatar-xs rounded-circle object-cover"></div>                    <div class="flex-grow-1 ms-2 name">' + customerNameField.value + "</div>                    </div>",
            company_name: company_nameField.value,
            designation: designationField.value,
            email_id: email_idField.value,
            phone: phoneField.value,
